...
########################################################################################################
# 1. AVERAGE MONTHLY COUNTY VALUES
########################################################################################################

print('processing mmean')

var <- paste0('mmean_',dname)

# process for finding average temperature
dat.at <- dat.county
names(dat.at)[grep(dname,names(dat.at))] <- 'variable'
dat.at <- ddply(dat.at,.(year,month,state.county.fips),summarize,mmean=round(mean(variable),2))
...
# save output
dir.output = paste0("../../output/metrics_development_county_era5/",dname,'/',var,'/')
...
dir.output = paste0("../../output/metrics_development_county_era5/",dname,'/',var,'/plots/')
ifelse(!dir.exists(dir.output), dir.create(dir.output, recursive=TRUE), FALSE)

library(ggplot2)
pdf(paste0(dir.output,year,'.pdf'),height=0,width=0,paper='a4r')
ggplot(data=subset(dat.at,!(state.fips%in%c('02','15'))&month==1)) +
    geom_point(aes(x=state.county.fips,y=mmean)) +
