...
directory = paste0("../../output/multiyear_normals_plots_era5/",dname,"/",metric)
...
# load data
dat = readRDS(paste0("../../output/multiyear_normals_era5/",dname,"/",metric,'/county_longterm_nonnormals_',var,'_',year.start,'_',year.end,'.rds'))
...
dat$fips= substr(dat$state.county.fips,1,2)
dat$fips = as.numeric(as.character(dat$fips))
...
#     geom_point(aes(x=code_name,y=t2m.38yr.mean)) +
...
ggplot(data=subset(dat,!(fips%in%c(2,15)))) +
    geom_point(aes(x=month.short,y=t2m.38yr.mean)) +
    geom_hline(yintercept=0,linetype='dotted') +
